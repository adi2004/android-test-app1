...
protected LogTable logTable= new LogTable();
 ...
 */
  ...
  /**
   * Die iRPImage Klasse wurde fuer dieses Applet erweitert,da
   * die Ausgabe im Houghraum logarithmiert erfolgen soll.
   * @param log Gibt an,ob die Ausgabe logarithmiert erfolgen soll oder nicht.
   */
  ...
  /**
   * Berechnet den Logarithmus der Bilddaten.
   *
   */
  public void takeTheLogarithm(){
      for(int i=0;i<sizeX*sizeY;i++){
          logImage[i]=(int)Math.round(((logTable.getLogarithm(image[i]+1))/log2));
